...
IndexTTS-2.5 WebUI 将使用和制作分成两个页签。“音频生成”页只允许从已安装音色包下拉菜单选择，或拖入已有 `.ivp`；选中后直接使用预计算张量，不会读取或编码参考 WAV。“音色包制作”页单独提供参考音频、音色 ID、名称、性别和生成下载入口；制作完成后会自动安装并选中。拖入或制作的包都必须先通过 schema、哈希和源模型指纹校验。
